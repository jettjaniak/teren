...
# perturbation_layer = "blocks.11.hook_resid_post"
seed = 71
...
class ReconRandomActivationPerturbation(Perturbation):
    """Reconstruction of random activation direction"""

    def __init__(self, model: HookedTransformer, dataset: Dataset, sae: SAE):
        self.model = model
        self.dataset = dataset
        self.sae = sae
...
class ActivateOneSAEFeaturePerturbation(Perturbation):
    ...
    def __init__(self, sae, sparse_map, activation_value):
        self.sae = sae
        self.true_indices = list(sparse_map.keys())
...
def scan(
    ...
    all_cache.append(perturbed_steps)
...
for data_point in data:
    data_norms.append(sae.encode(data_point).norm())
mean_norm = np.mean(data_norms)
mean_norm = torch.tensor(mean_norm, dtype=torch.float32)
...
perturbations["other-act"] = r_other_perturbation

activate_one_sae_feature_perturbation = ActivateOneSAEFeaturePerturbation(
    sae, sparse_map, mean_norm
)
perturbations["single"] = activate_one_sae_feature_perturbation
...
perturbations["recon-other-act"] = recon_r_other_perturbation
...
# %%
print("Norm of the target activation T for each perturbation type")
for i in range(len(results.keys())):
    temp = [all_cache[len(results.keys()) * j + i] for j in range(num_runs)]
    final_norms = [t[-1].norm() for t in temp]
    print(list(results.keys())[i], np.mean(final_norms))

print("Norm of the full perturbation D=(T-A) for each perturbation type")
for i in range(len(results.keys())):
    temp = [all_cache[len(results.keys()) * j + i] for j in range(num_runs)]
    D_norms = [(t[-1] - t[0]).norm() for t in temp]
    print(list(results.keys())[i], np.mean(D_norms))

print("Mean of the full perturbation D=(T-A) for each perturbation type")
for i in range(len(results.keys())):
    temp = [all_cache[len(results.keys()) * j + i] for j in range(num_runs)]
    D_norms = [(t[-1] - t[0]).mean() for t in temp]
    print(list(results.keys())[i], np.mean(D_norms))

print("Mean of the base activation A for each perturbation type")
for i in range(len(results.keys())):
    temp = [all_cache[len(results.keys()) * j + i] for j in range(num_runs)]
    D_norms = [(t[0]).mean() for t in temp]
    print(list(results.keys())[i], np.mean(D_norms))

print("Mean of the target activation T for each perturbation type")
for i in range(len(results.keys())):
    temp = [all_cache[len(results.keys()) * j + i] for j in range(num_runs)]
    D_norms = [(t[-1]).mean() for t in temp]
    print(list(results.keys())[i], np.mean(D_norms))
# %%
...
colors = ["blue", "orange", "red", "green", "brown", "purple", "lightgray"]

# Create a color mapping dictionary
color_mapping = {name: colors[i % len(colors)] for i, name in enumerate(perturb_names)}


def plot_steps(blowup_step, step_type, metric, n_steps=n_steps):
...
for perturbation_name in perturb_names:
    print(
        f"KS distance for {perturbation_name} is : {kstest(blowup_step_l2[perturbation_name], blowup_step_l2['other-act'])[0]}"
    )

for key in metrics_max_step_l2.keys():
    print(
        f"for {key} mean: {metrics_max_step_l2[key]['mean']}, std: {metrics_max_step_l2[key]['variance']}"
    )
...
if save_results:
    ...
    with open("All_perts_base_A_layer_1_jb.json", "w") as file:
