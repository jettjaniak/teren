...
class CompositeRandomSAEFeaturePerturbation(Perturbation):
    """We create composite synthetic activation using random non-dead features activated to the same value as the base"""
    ...
    def generate(self, resid_acts):
        ...
        for _ in range(len(base_encoded_actives)):
            while True:
                ...
                if (
                    random_index not in random_indices
                    and base_encoded[0][0][random_index] == 0
                ):
...
recon_perturbations = {}
# recon_random_perturbation = ReconRandomPerturbation(data_mean, data_cov, sae)
# recon_perturbations["recon_random"] = recon_random_perturbation

random_perturbation = RandomPerturbation(data_mean, data_cov)
recon_perturbations["recon_to_random"] = random_perturbation

recon_r_other_perturbation = ReconRandomActivationPerturbation(model, dataset, sae)
recon_perturbations["recon_r_other"] = recon_r_other_perturbation

activate_one_sae_feature_perturbation = ActivateOneSAEFeaturePerturbation(
    sae, feature_map, mean_norm
)
recon_perturbations["single_feature_sae"] = activate_one_sae_feature_perturbation


composite_random_sae_feature_perturbation = CompositeRandomSAEFeaturePerturbation(
    sae, feature_map
)
recon_perturbations["composite_random_sae"] = composite_random_sae_feature_perturbation
torch.cuda.empty_cache()
print(f"Allocated: {torch.cuda.memory_allocated() / 1024 ** 3:.2f} GB")
...
for _ in tqdm(range(num_runs)):
    ...
    for name, perturbation in recon_perturbations.items():
