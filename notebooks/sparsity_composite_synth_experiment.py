...
class CompositeRandomSAEFeaturePerturbation(Perturbation):
    ...
    def __init__(self, sae, feature_map):
        self.sae = sae
        self.true_indices = torch.nonzero(feature_map, as_tuple=False).squeeze()
...
KS_random = kstest(blowup_step_l2["random"], blowup_step_l2["r_other"])
KS_composite_sparsity = kstest(
    blowup_step_l2["composite_sparsity"], blowup_step_l2["r_other"]
)
print(KS_random[0])
print(KS_composite_sparsity[0])
...
if save_results:
    ...
    with open("sparsity_composite_synth_experiment_results.json", "w") as file:
...
if read_results:
    with open("sparsity_composite_synth_experiment_results.json", "r") as file:
