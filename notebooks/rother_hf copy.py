...
mean_dist_max_diff = dist.max(dim=0).values - dist.min(dim=0).values
q90 = torch.quantile(mean_dist_max_diff, 0.85)
q10 = torch.quantile(mean_dist_max_diff, 0.15)
print(f"{q90=}")
...
ax[0].plot(ls.tolist()[::-1], dist_[:, argmin > 14], color="blue", alpha=0.2)
ax[0].plot(ls.tolist()[::-1], dist_[:, argmin <= 6], color="red", alpha=0.2)
ax[0].set_title(f"$|{distance}-[\\alpha B+(1-\\alpha)A]|_2$ after L{layer_pert}")
ax[0].set_xlabel("$\\alpha$")

ax[1].plot(ls.tolist()[::-1], nnorms[:, argmin > 14], color="blue", alpha=0.3)
ax[1].plot(ls.tolist()[::-1], nnorms[:, argmin <= 6], color="red", alpha=0.3)
